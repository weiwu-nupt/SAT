...
class UDPServer:
    ...
    def send_net_lora_message(self, message_type: int, content: bytes, 
        ...
        try:
            ...
            if message_length > 255:
                logger.error(f"消息内容过长: {message_length} > 255")
                return False
    ...
    def broadcast_message(self, message_type: int, content: bytes, 
        ...
        try:
            ...
            logger.info(f"广播消息完成，成功: {success_count}/2")
...
class MessageProcessor:
    ...
    def process_message(self, message: ParsedMessage):
        ...
        if debug_config.enable_debug:
            logger.debug(f"  原始数据: {message.raw_data}")
            if debug_config.print_hex:
                logger.debug(f"  十六进制: {message.raw_data.hex()}")
    ...
    def _send_response(self, message: ParsedMessage):
        ...
        try:
            if message.source == MessageSource.PLATE:
                # 回复Plate消息
                response = b"Plate response: " + message.content[:10]  # 回复前10字节
...
def main():
    ...
    if not load_config_from_file("config.ini"):
        ...
        logger.info(f"本机节点ID: {local_id}")
        logger.info(f"网络模式: {sys_para.net_mod} (0-6:物联网模式, 7:簇首调度, 8:自组织网)")
        logger.info(f"节点模式: {sys_para.node_mod} (0:母星, 1:簇首, 2:普通节点)")
        logger.info(f"本机IP: {network_config.local_ip}")
        logger.info(f"MAC地址: {[hex(x) for x in node_para.mac_addr]}")
    ...
    try:
        ...
        if debug_config.enable_debug or system_config.log_level.upper() == 'DEBUG':
            ...
            while True:
                ...
                logger.info(f"当前队列消息数: {queue_size}")
                logger.debug(f"系统参数 - 网络模式:{sys_para.net_mod}, 节点数:{sys_para.node_num}")
        else:
            # 生产模式下静默等待，只在有消息时才输出
            try:
                while True:
                    time.sleep(10)  # 较长的睡眠间隔
                    # 可以添加一些必要的系统检查
                    if not processor.running or not server.running:
                        logger.warning("检测到服务异常停止")
                        break
