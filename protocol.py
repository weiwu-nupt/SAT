#!/usr/bin/env python3
# -*- coding: utf-8 -*-

import struct
import logging
from enum import Enum
from dataclasses import dataclass
from datetime import datetime
from typing import Optional, Dict, Any


class MessageProtocol:
    """消息协议处理类"""
    
class MessageProtocol:
    """消息协议处理类"""
    
    # CRC16查找表，基于多项式0x1021
    CRC16_TABLE = [
        0x0000, 0xC0C1, 0xC181, 0x0140, 0xC301, 0x03C0, 0x0280, 0xC241,
        0xC601, 0x06C0, 0x0780, 0xC741, 0x0500, 0xC5C1, 0xC481, 0x0440,
        0xCC01, 0x0CC0, 0x0D80, 0xCD41, 0x0F00, 0xCFC1, 0xCE81, 0x0E40,
        0x0A00, 0xCAC1, 0xCB81, 0x0B40, 0xC901, 0x09C0, 0x0880, 0xC841,
        0xD801, 0x18C0, 0x1980, 0xD941, 0x1B00, 0xDBC1, 0xDA81, 0x1A40,
        0x1E00, 0xDEC1, 0xDF81, 0x1F40, 0xDD01, 0x1DC0, 0x1C80, 0xDC41,
        0x1400, 0xD4C1, 0xD581, 0x1540, 0xD701, 0x17C0, 0x1680, 0xD641,
        0xD201, 0x12C0, 0x1380, 0xD341, 0x1100, 0xD1C1, 0xD081, 0x1040,
        0xF001, 0x30C0, 0x3180, 0xF141, 0x3300, 0xF3C1, 0xF281, 0x3240,
        0x3600, 0xF6C1, 0xF781, 0x3740, 0xF501, 0x35C0, 0x3480, 0xF441,
        0x3C00, 0xFCC1, 0xFD81, 0x3D40, 0xFF01, 0x3FC0, 0x3E80, 0xFE41,
        0xFA01, 0x3AC0, 0x3B80, 0xFB41, 0x3900, 0xF9C1, 0xF881, 0x3840,
        0x2800, 0xE8C1, 0xE981, 0x2940, 0xEB01, 0x2BC0, 0x2A80, 0xEA41,
        0xEE01, 0x2EC0, 0x2F80, 0xEF41, 0x2D00, 0xEDC1, 0xEC81, 0x2C40,
        0xE401, 0x24C0, 0x2580, 0xE541, 0x2700, 0xE7C1, 0xE681, 0x2640,
        0x2200, 0xE2C1, 0xE381, 0x2340, 0xE101, 0x21C0, 0x2080, 0xE041,
        0xA001, 0x60C0, 0x6180, 0xA141, 0x6300, 0xA3C1, 0xA281, 0x6240,
        0x6600, 0xA6C1, 0xA781, 0x6740, 0xA501, 0x65C0, 0x6480, 0xA441,
        0x6C00, 0xACC1, 0xAD81, 0x6D40, 0xAF01, 0x6FC0, 0x6E80, 0xAE41,
        0xAA01, 0x6AC0, 0x6B80, 0xAB41, 0x6900, 0xA9C1, 0xA881, 0x6840,
        0x7800, 0xB8C1, 0xB981, 0x7940, 0xBB01, 0x7BC0, 0x7A80, 0xBA41,
        0xBE01, 0x7EC0, 0x7F80, 0xBF41, 0x7D00, 0xBDC1, 0xBC81, 0x7C40,
        0xB401, 0x74C0, 0x7580, 0xB541, 0x7700, 0xB7C1, 0xB681, 0x7640,
        0x7200, 0xB2C1, 0xB381, 0x7340, 0xB101, 0x71C0, 0x7080, 0xB041,
        0x5000, 0x90C1, 0x9181, 0x5140, 0x9301, 0x53C0, 0x5280, 0x9241,
        0x9601, 0x56C0, 0x5780, 0x9741, 0x5500, 0x95C1, 0x9481, 0x5440,
        0x9C01, 0x5CC0, 0x5D80, 0x9D41, 0x5F00, 0x9FC1, 0x9E81, 0x5E40,
        0x5A00, 0x9AC1, 0x9B81, 0x5B40, 0x9901, 0x59C0, 0x5880, 0x9841,
        0x8801, 0x48C0, 0x4980, 0x8941, 0x4B00, 0x8BC1, 0x8A81, 0x4A40,
        0x4E00, 0x8EC1, 0x8F81, 0x4F40, 0x8D01, 0x4DC0, 0x4C80, 0x8C41,
        0x4400, 0x84C1, 0x8581, 0x4540, 0x8701, 0x47C0, 0x4680, 0x8641,
        0x8201, 0x42C0, 0x4380, 0x8341, 0x4100, 0x81C1, 0x8081, 0x4040
    ]
    
    @staticmethod
    def calculate_crc16(data: bytes) -> int:
        """计算CRC16校验码"""
        crc = 0xFFFF  # CRC初始值
        
        for byte in data:
            # 按照C代码的实现方式
            crc = (crc << 8) ^ MessageProtocol.CRC16_TABLE[(crc >> 8) ^ byte]
            crc &= 0xFFFF  # 保持16位
        
        return crc
    
    @staticmethod
    def pack_message(msg_type: int, content: bytes) -> bytes:
        """封装消息
        格式: [消息类型(1字节)] [消息长度(1字节)] [消息内容(若干字节)] [CRC(2字节)]
        """
        if len(content) > 253:  # 限制内容长度，为CRC预留空间
            raise ValueError("消息内容长度不能超过253字节")
        
        # 构建消息头部
        msg_len = len(content) + 4  # 总长度 = 类型(1) + 长度(1) + 内容长度 + CRC(2)
        header = struct.pack('BB', msg_type, msg_len)
        
        # 构建待校验的数据（不包括CRC）
        data_to_crc = header + content
        
        # 计算CRC
        crc = MessageProtocol.calculate_crc16(data_to_crc)
        
        # 封装完整消息，CRC按大端序（高字节在前）
        message = data_to_crc + struct.pack('>H', crc)
        
        return message
    
    @staticmethod
    def unpack_message(raw_data: bytes) -> tuple:
        """解包消息
        返回: (消息类型, 消息内容, 是否校验成功)
        """
        if len(raw_data) < 4:  # 最小消息长度
            return None, None, False
        
        try:
            # 解析消息头
            msg_type = raw_data[0]
            msg_len = raw_data[1]
            
            # 检查长度是否匹配
            if len(raw_data) != msg_len:
                return None, None, False
            
            # 提取内容和CRC
            content = raw_data[2:-2]
            received_crc = struct.unpack('>H', raw_data[-2:])[0]  # 大端序读取CRC
            
            # 计算CRC校验
            data_to_crc = raw_data[:-2]
            calculated_crc = MessageProtocol.calculate_crc16(data_to_crc)
            
            # 验证CRC
            crc_valid = (received_crc == calculated_crc)
            
            return msg_type, content, crc_valid
            
        except Exception as e:
            logging.error(f"消息解包失败: {e}")
            return None, None, False


class MessageType:
    """消息类型常量定义"""
    # LoRa消息类型
    LORA_BROADCAST = 0x01          # 系统广播消息
    LORA_REGISTER_REQ = 0x02       # 入网注册请求
    LORA_REGISTER_RSP = 0x03       # 入网注册应答
    LORA_NODE_CONTROL = 0x04       # 节点控制消息
    LORA_CLUSTER_OP = 0x05         # 簇操作消息
    LORA_RANGING_REQ = 0x11        # 测距调度消息
    LORA_RANGING_APPLY = 0x12      # 测距申请消息
    LORA_RANGING_RSP = 0x13        # 测距应答消息
    LORA_INQUIRY = 0x20            # 询问消息
    LORA_INQUIRY_RSP = 0x21        # 询问应答消息
    LORA_TRAFFIC_DATA = 0x22       # 业务数据消息
    LORA_NODE_STATUS = 0x23        # 节点状态消息
    LORA_CLUSTER_STATUS = 0x25     # 簇状态消息
    LORA_PLATFORM_DATA = 0x26      # 平台数据消息
    LORA_LINK_DATA = 0x27          # 链路数据消息
    LORA_MOTHER_SWITCH = 0x30      # 母星切换消息
    LORA_MOTHER_ELECTION = 0x31    # 母星选举消息
    
    # 控制消息类型
    CMD_SET_NODE_TYPE = 0x01       # 设置节点类型
    CMD_SET_NETWORK_CONFIG = 0x02  # 设置网络参数
    CMD_QUERY_STATUS = 0x03        # 查询节点状态
    CMD_START_NETWORK = 0x04       # 启动组网
    CMD_STOP_NETWORK = 0x05        # 停止组网
    
    # 网管消息类型
    NETMGR_NETWORK_MGMT = 0x10     # 网络管理消息
    NETMGR_NODE_CONFIG = 0x11      # 节点配置消息
    NETMGR_DATA_FORWARD = 0x12     # 数据转发消息


class MessageSource(Enum):
    """消息来源枚举"""
    HOST_SOFTWARE = "host_software"
    GATEWAY = "gateway"
    LORA = "lora"


@dataclass
class UDPMessage:
    """UDP消息数据类"""
    source: MessageSource
    data: bytes
    addr: tuple
    timestamp: datetime
    msg_type: Optional[int] = None  # 添加消息类型字段
    
    def to_dict(self) -> Dict[str, Any]:
        return {
            'source': self.source.value,
            'msg_type': f"0x{self.msg_type:02X}" if self.msg_type else None,
            'data': self.data.hex() if isinstance(self.data, bytes) else str(self.data),
            'data_length': len(self.data),
            'addr': self.addr,
            'timestamp': self.timestamp.isoformat()
        }